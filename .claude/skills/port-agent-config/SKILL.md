...
| GitHub Copilot, Gemini CLI, Zed | nothing to generate — none support true sub-agent delegation, only persona/tool-scope switching for the same agent (see `docs/CLIENT_COMPATIBILITY.md`'s custom-agent table) | — |

**Important limitation, state it every time**: none of these four
generators port tool/permission scoping (Claude Code's `tools:` field,
Cursor's `readonly`/`is_background`, Kilo's `permission`/
`permission.task`) — that vocabulary is unverified per-platform. Ported
...
6. **Custom subagents** (task delegation, not skills/instructions) only
   ...
   (Claude Code, Codex CLI, OpenCode, Cursor, Kilo Code — see
   `docs/CLIENT_COMPATIBILITY.md`'s custom-agent table); porting to
   Copilot/Gemini CLI/Zed means falling back to a persona/mode switch,
   not delegation, and that difference must be said out loud, not
   silently smoothed over. Same tool/permission-scoping limitation as
   Section A's generators applies here too — port `name`/`description`/
   `model`/body, don't guess at the target's tool-name vocabulary.
