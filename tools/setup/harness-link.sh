...
# Writes a small consumer-local script at TARGET/.agentharness-bin/check
# that delegates to the resolved harness checkout's own 'enforce-profile'
# subcommand. This is the actual, runnable substitute for the harness-only
# tools/check-completion.sh (issue #110): committing/SKILL.md tells agents
# to run a completion gate, but that script is never shipped to consumer
# projects in any install mode -- and even where a copy physically exists
# (submodule mode ships the whole harness repo, check-completion.sh
# included), it resolves its OWN repo root via BASH_SOURCE, not the
# caller's cwd, so invoking it directly always tests the harness's own
# code, never the consumer's. This wrapper fixes both: it lives inside the
# consumer project and passes an explicit consumer TARGET argument to
# enforce-profile.
#
# Only submodule/npm/link modes get a wrapper. copy mode never keeps a
# live harness-link.sh anywhere near the consumer (skills are physically
# copied, nothing else is) -- there is no script left for a wrapper to
# delegate to, so none is generated; doctor/audit report this explicitly
# rather than silently omitting it. link mode's wrapper bakes in
# SKILLS_SRC_ROOT (HARNESS_DIR) as an absolute path at generation time --
# the same "external, unpinned path" caveat link mode already has
# everywhere else (see docs/RELEASING.md's Pin/Upgrade/Rollback table).
generate_check_wrapper() {
    local target="$1" mode="$2" skills_src_root="$3"
    local wrapper_path
...
#!/usr/bin/env bash
# Generated by agentharness harness-link.sh -- do not edit by hand.
# Re-run 'harness-link.sh init'/'update' to regenerate.
#
# The consumer-local completion gate (issue #110): delegates to the
# harness checkout this project was installed from ('$mode' mode),
# running its language-aware test+coverage enforcement against THIS
# project (not the harness's own code -- see the comment on
# generate_check_wrapper() in tools/setup/harness-link.sh for why that
# distinction needed a generated wrapper instead of just documentation).
set -euo pipefail
project_root="\$(cd "\$(dirname "\${BASH_SOURCE[0]}")/.." && pwd)"
exec "$skills_src_root/tools/setup/harness-link.sh" enforce-profile "\$project_root" "\$@"
EOF
...
cmd_init() {
    ...
    if [ "$dry_run" = true ]; then
        ...
        if [ "$mode" = "copy" ]; then
            echo "  Check wrapper: none for --mode copy (no live harness checkout stays reachable)"
        else
            echo "  Check wrapper: generate $CHECK_WRAPPER_DIR/$CHECK_WRAPPER_NAME"
        fi
    ...
    # ------------------------------------------------------------------
    # 4b. Consumer-local completion gate (issue #110)
    # ------------------------------------------------------------------
    generate_check_wrapper "$target" "$mode" "$skills_src_root"
...
" || failed=1
    ...
    # issue #110: soft-warn (not fail) if the consumer-local completion
    # gate is missing or not executable for a mode that should have one.
    # Not a hard failure: installs that predate this feature won't have a
    # wrapper until 'update' runs once, and that's a normal, recoverable
    # state, not a broken one.
    local doctor_mode
    doctor_mode="$(state_field "$target" mode)"
    if [ "$doctor_mode" != "copy" ]; then
        local doctor_wrapper_path
        doctor_wrapper_path="$(echo_check_wrapper_path "$target")"
        if [ ! -x "$doctor_wrapper_path" ]; then
...
cmd_audit() {
    ...
    # consumer needs before starting work -- publish authority (already
    # covered above by publish_mode_active), hook state, helper-command
    # (the check wrapper) availability, and whether this install can
    # actually mechanically enforce the workflow its policy text describes.
    local with_hook coverage_hook core_hooks_path
    with_hook="$(state_field "$target" with_hook 2>/dev/null || echo "false")"
    coverage_hook="$(state_field "$target" coverage_hook 2>/dev/null || echo "false")"
...
PYEOF
    ...
    if [ "$mode" = "copy" ]; then
        ...
        echo "Consumer-local completion gate: $wrapper_path"
    else
        echo "Consumer-local completion gate: missing — run 'update' to generate $wrapper_path"
    fi
...
cmd_uninstall() {
    ...
    local check_wrapper_path
    check_wrapper_path="$(echo_check_wrapper_path "$target")"
    if [ -f "$check_wrapper_path" ]; then
        rm -f "$check_wrapper_path"
        rmdir "$(dirname "$check_wrapper_path")" 2>/dev/null || true
        echo "  Removed $CHECK_WRAPPER_DIR/$CHECK_WRAPPER_NAME"
    fi
