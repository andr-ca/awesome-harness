...
| AC-10 | Profile operations support preview, apply, validate, and explain. | 1 | `cli.py`, `profile/history.py` | `tests/integration/test_profile_commands.py` | CLI JSON-schema snapshots | planned |
...
| AC-14 | Evidence invalidates after every specified relevant input change. | 3 | `policy/fingerprint.py`, `policy/evidence.py` | `tests/unit/policy/test_fingerprint.py`; `tests/integration/test_evidence_inputs.py` | Invalidation coverage report with all input classes | planned |
...
| AC-30 | Policy namespace coexists with link, copy, npm, and `.agentharness/` submodule mode. | 1, 6 | lifecycle migration and paths | expanded fixture matrix | Four-mode install/bootstrap/update/uninstall report | planned |
