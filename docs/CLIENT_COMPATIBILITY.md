...
| Cursor | `.cursorrules` (legacy, always-on) or `.cursor/rules/*.mdc` (current; per-file `alwaysApply`/`globs`/`description` activation) | `.cursorrules` loads unconditionally; `.mdc` files load per their own frontmatter | ✅ `.cursor/rules/agentharness-router.mdc` + one `.mdc` per skill, generated by `tools/generate-cursor-rules.sh` |
...
| Cursor | none — no confirmed SKILL.md support | Not adopted; uses `.mdc` rules instead | ✅ one `.cursor/rules/<skill>.mdc` per skill (Agent-Requested activation), generated by `tools/generate-cursor-rules.sh` |
