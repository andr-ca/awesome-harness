...
| pre-merge-commit dispatcher | `.github/hooks/pre-merge-commit` | git hook | Required to block merge commits to trunk branches — `core.hooksPath` only invokes a file literally named `pre-merge-commit`; git only falls back to `pre-commit` for merge commits under certain conditions, and when that fallback fails, merge commits bypass protection entirely (see issue #76) |
...
| GitHub issue triage | `.claude/skills/github-issue-triage/SKILL.md` | skill | Triage GitHub issues by verifying claims against running code, assessing roadmap fit, and posting structured findings with recommendations |
...
| Bootstrap policy core (⚠ EXPERIMENTAL — unreleased; not wired into public CLI) | `src/agentharness/` | package | Deterministic Python policy core (bootstrap, gates, profiles, plugins) — experimental and unreleased; the public npm launcher does not invoke it. See docs/STATUS.md for the version split and E1/E2 in the 2026-07-16 launch-readiness plan |
...
| Existing-surface integration design (implemented) | `docs/superpowers/specs/2026-07-17-existing-surface-integration-design.md` | design specification |
| Existing-surface integration implementation plan (implemented) | `docs/superpowers/plans/2026-07-17-existing-surface-integration.md` | implementation plan |
